...
class RankingRepository:
    def __init__(self, dsn: Optional[str] = None):
        ...
        if not self.dsn:
            logger.warning("POSTGRES_DSN not set. RankingRepository will not function.")

    def get_ranking_schema(self, name: str) -> Optional[dict[str, Any]]:
        if not self.dsn:
            return None
    ...
    def save_ranking_schema(self, *, name: str, config: dict[str, Any], description: str = "") -> dict[str, Any]:
        if not self.dsn:
            raise ValueError("Database connection not configured")
        with connect(self.dsn) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"SELECT version FROM {RANKING_SCHEMAS_TABLE} WHERE name = %s",
                    (name,),
                )
                row = cur.fetchone()
    ...
    def delete_ranking_schema(self, name: str) -> bool:
        ...
        with connect(self.dsn) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"DELETE FROM {RANKING_SCHEMAS_TABLE} WHERE name = %s RETURNING name",
                    (name,),
                )
