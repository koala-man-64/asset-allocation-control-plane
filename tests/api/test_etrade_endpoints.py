...
@pytest.mark.asyncio
async def test_etrade_callback_url_route_returns_resolved_url_when_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    _configure_oidc(monkeypatch)
    monkeypatch.setenv("ETRADE_ENABLED", "false")
    ...
    async with get_test_client(app) as client:
        response = await client.get(
            "/api/providers/etrade/connect/callback-url",
            headers={"Authorization": "Bearer placeholder"},
        )

    assert response.status_code == 200
    assert response.json() == {"callback_url": "https://api.example.com/api/providers/etrade/connect/callback"}


@pytest.mark.asyncio
async def test_etrade_callback_url_route_returns_503_when_unresolved(monkeypatch: pytest.MonkeyPatch) -> None:
    _configure_oidc(monkeypatch)
    ...
    assert response.json()["detail"] == (
        "E*TRADE callback URL is not configured. Set ETRADE_CALLBACK_URL or API_PUBLIC_BASE_URL."
    )
