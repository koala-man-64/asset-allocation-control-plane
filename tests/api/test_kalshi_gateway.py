...
def _settings_with_live_only() -> KalshiSettings:
...
def test_gateway_resolves_environment_specific_credentials() -> None:
    ...
    assert demo.api_key_id is None
    assert demo.private_key_pem is None
...
def test_gateway_keeps_unconfigured_environment_explicit() -> None:
    gateway = KalshiGateway(_settings_with_live_only())
